...
HorizonAnneal3<-function(d3,param=list(nouter=400,ninner=100,temperature=5,cooling=0.9),pen_str)
{
	#  simulated annnealing based on biostratigraphy+chemostratigraphy
	# d3[,1] - xpositions
	# d3[,2] -section codes
	# d3[,2+1:lastcolumn] - geological variables- of all types
  ...
  {
    cv=NetRangeExtension(d3a[,2+pen_str$biostrat])
  }
  else
  {
  	cv=0
  ...
  if(pen_str$n_pmag==1)
  {
  		    cv=cv+ftransitions2(d3a[,pen_str$pmag+2])*30    
  }	
  else
  {
  	  if(pen_str$n_pmag>1)
  	  {
  	  	cv=cv+sum(apply(d3a[,pen_str$pmag+2],2,ftransitions2))*30
  	  }
  }
  				# compute the AshRangeExtensionPenalty
  if(pen_str$n_ashes==1)
  {
  		    cv=cv+AshRangeExtension(d3a[,pen_str$ashes[,1]+2])*10  
  ...
  else
  {
  	   if(pen_str$n_ashes>1)
  	   {
  	   	  cv=cv+sum(apply(d3a[,pen_str$ashes[,1]+2],2,AshRangeExtension))*10  
  	   }
  }
  ...
  {
  		    cv=cv+ftransitions2(d3a[,pen_str$continuous[,1]+2])*pen_str$continuous[,2]   
  }
  else
  {
  	if(pen_str$n_continuous>1)
  	{
  			...
  			cv=cv+sum(apply(d3a[,pen_str$continuous[,1]+2],2,ftransitions2))*pen_str$continuous[1,2]
  			cat(cv)
  	}
  }
  ...
  if(pen_str$n_dates>0)
  {
  		...
  		{
  		    cv=cv+PassingError2(d3a[,pen_str$dates[i,1]+2],pen_str$dates[i,2],d3a[,pen_str$dates[i,3]+2],pen_str$dates[i,4])*pen_str$dates[i,5]
...
#		}
		...
		if(pen_str$n_biostrat>0)
  		{
    		pcv3=NetRangeExtension(pd3[,2+pen_str$biostrat])  		}
  		else
  		{
  			pcv3=0
  		}
  		...
  		if(pen_str$n_pmag==1)
  		{
  		    pcv3=pcv3+ftransitions2(pd3[,pen_str$pmag+2])*30    
  		}	
  		else
  		{
  	  		if(pen_str$n_pmag>1)
  	  		{
  	  			pcv3=pcv3+sum(apply(pd3[,pen_str$pmag+2],2,ftransitions2))*30
  	  		}
  		}
  		...
  		if(pen_str$n_ashes==1)
  		{
  		    pcv3=pcv3+AshRangeExtension(pd3[,pen_str$ashes[,1]+2])*10    
  		}
  		else
  		{
  	   		if(pen_str$n_ashes>1)
  	   		{
  	   	  		pcv3=pcv3+sum(apply(pd3[,pen_str$ashes[,1]+2],2,AshRangeExtension))*10  
  	   		}
  		}
		...
		if(pen_str$n_continuous>0)
  		{
  			...
  			{
  		    	pcv3=pcv3+ftransitions2(pd3[,i+2])*pen_str$continuous[1,2]
  			}    
  		}
		...
		if(pen_str$n_dates>0)
  		{
  			for(i in 1:pen_str$n_dates)
  			{
  		    	pcv3=pcv3+PassingError2(pd3[,pen_str$dates[i,1]+2],pen_str$dates[i,2],pd3[,pen_str$dates[i,3]+2],pen_str$dates[i,4])*pen_str$dates[i,5]
  			}    
  		}
...
if(pen_str$n_pmag>0)
 {
   ...
   {
      pcv3=pcv3+ftransitions2(bestd3[,i+2])*30
   }    
 }	
...
if(pen_str$n_ashes>0)
  {
  		...
  		{
  		    acv=acv+AshRangeExtension(bestd3[,i+2])*10
  		}    
  }
		...
		if(pen_str$n_continuous>0)
  		{
  			for(i in pen_str$continuous[,1])
  			{
  		    	ccv=ccv+ftransitions2(bestd3[,i+2])
  			}    
  		}
		...
		if(pen_str$n_dates>0)
  		{
  			for(i in 1:pen_str$n_dates)
  			{
  		    	pcv=pcv+PassingError2(bestd3[,pen_str$dates[i,1]+2],pen_str$dates[i,2],bestd3[,pen_str$dates[i,3]+2],pen_str$dates[i,4])*pen_str$dates[i,5]
  			}    
  		}
